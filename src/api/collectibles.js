...
function readNullableString(value, fieldName) {
    if (value === null) {
        return null;
    }
...
function readOptionalNullableString(value) {
        ...
        products: products.map((product, index) => {
            ...
            return {
                ...
                prices: isRecord(prices)
                    ? Object.entries(prices).reduce((acc, [tierKey, tierValue]) => {
...
export async function fetchCollectibles() {
    const payload = await fetchJson(COLLECTIBLES_ENDPOINT);
